					...
					int flags)
{
	...
	if (!selinux_state.initialized) {
		/* If we haven't even been initialized, then we can't validate against a
		 * policy, so leave the label as invalid. It may resolve to a valid
		 * label on the next revalidation try if we've since initialized. */
		return;
	}
